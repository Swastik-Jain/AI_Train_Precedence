"""
baseline_rule.py — Rule-Based Dispatcher Baseline
Mimics what Indian Railways actually does today:
strict priority ordering, no learned behavior.
...
Rules:
    1. Highest priority train always gets main line
    2. Lower priority trains divert to loop if available
    3. If loop full — hold
    4. No anticipation of future conflicts
...
def rule_based_action(trains, track_map, node_km=None):
    """
    Generate actions using strict priority-order rule.
    Returns np.ndarray of actions shape (MAX_TRAINS_CAPACITY,)
    """
...
def run_rule_based(
    ...
    print(f"\n{'='*60}")
    print(f"RULE-BASED BASELINE | {num_trains} trains | {num_episodes} episodes")
    print(f"{'='*60}")
...
if __name__ == '__main__':
    run_rule_based(
        ...
        num_episodes=200,
        seed=42,
        save_path='results/rule_based.json',
    )
